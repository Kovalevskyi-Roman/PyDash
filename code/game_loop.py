...
from game_state import GameState, Menu, LevelList, Play, PropertyEditor, Editor, Settings
...
class GameLoop:
    def __init__(self, window: Window) -> None:
        ...
        menu = Menu()
        settings = Settings(self.__window)
        level_list = LevelList(self.__tile_manager)
        play = Play(self.__tile_manager, level_list, settings)
        ...
        self.__font = pygame.Font("../resources/fonts/regular.ttf", 12)
        self.__running = True
    ...
    def __draw(self) -> None:
        ...
        if self.__game_states.get("settings").show_fps.value:
            rendered_fps = self.__font.render(f"fps:{int(self.__window.clock.get_fps())}", True, "white", 0)
            self.__window.surface.blit(rendered_fps, [0, 0])
